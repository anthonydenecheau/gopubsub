...
func (t *Task) publishPedigreeChange(message []*model.Pedigree, action string) (serverID string, err error) {
	event := struct {
		Type      string            `json:"type"`
		Action    string            `json:"action"`
		Message   []*model.Pedigree `json:"message"`
...
func (t *Task) sendPedigreeMessage(message []*model.Pedigree, action string) (serverID string, err error) {
	...
	if len(message) > 0 {
		...
		case action == "U":
			...
			serverID, err := t.publishPedigreeChange(message, "UPDATE")
			if err != nil {
				return "", err
			}
			return serverID, nil
		case action == "I":
			...
			serverID, err := t.publishPedigreeChange(message, "SAVE")
			if err != nil {
				return "", err
			}
			return serverID, nil
		case action == "D":
			...
			serverID, err := t.publishPedigreeChange(message, "DELETE")
			if err != nil {
				return "", err
			}
			return serverID, nil
...
func (d *pubTask) syncPedigreeChanges() {

	fmt.Println("Scanning table ws_dog_sync_data : filter applied ", d.pedigreeService.GetFilter())
	d.log.Infof("Scanning table ws_dog_sync_data : filter applied %s", d.pedigreeService.GetFilter())

	pedigreeList, err := d.dr.GetAllChanges(d.pedigreeService.GetFilter())
	if err != nil {
	...
	list := []messages{}
	// [[Boucle]] s/ le chien
	for _, pedigree := range pedigreeList {

		idDog := pedigree.ID
		action := pedigree.Action
		...
		message, err := d.pedigreeService.BuildMessage(idDog, action)
		if err != nil {
			fmt.Println("   >>  error", err)
			d.log.Errorf("   >>  error %s", err)
			continue
		...
		*/
		item := messages{pedigrees: message, action: action}
	...
	if len(list) > 0 {
		...
		for _, v := range list {
			...
			serverID, err := d.sendPedigreeMessage(v.pedigrees, v.action)
			if err != nil {
				d.log.Errorf("   >>  error %s", err)
			}
			d.log.Infof("   >>  server ID message %s", serverID)
